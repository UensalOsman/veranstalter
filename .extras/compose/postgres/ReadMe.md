...
Windows das Verzeichnis `C:/Zimmermann/volumes/postgres/tablespace/veranstalter` anlegen
...
```powershell
   ...
   docker compose exec db bash
      ...
      chown postgres:postgres /var/lib/postgresql/tablespace/veranstalter
...
neue DB `veranstalter` mit dem DB-User `veranstalter`anzulegen. Mit dem 2. Skript wird das
Schema `veranstalter` mit dem DB-User `veranstalter` als _Owner_ angelegt:

```powershell
   docker compose exec db bash
      psql --dbname=postgres --username=postgres --file=/sql/create-db-veranstalter.sql
      psql --dbname=veranstalter --username=veranstalter --file=/sql/create-schema-veranstalter.sql
      psql --dbname=veranstalter --username=veranstalter --file=/sql/create-table-veranstalter.sql
      psql --dbname=veranstalter --username=postgres --file=/sql/copy-csv-veranstalter.sql
      exit
    docker compose down
```
...
_Kopien_ der SQL-Skripte `create-db-veranstalter.sql` und `create-schema-veranstalter.sql` sind
...
Mit der Erweiterung _PostgreSQL_ für VS Code kann man die Datenbank `veranstalter` und
...
Im Untermenü _Databases_ von der Verbindung sieht man dann z.B. die Datenbank `veranstalter`
mit dem gleichnamigen Schema `veranstalter` und die Datenbank `postgres`.
Ebenso kann man man unter _Roles_ den DB-User `veranstalter` und den Superuser `postgres`
